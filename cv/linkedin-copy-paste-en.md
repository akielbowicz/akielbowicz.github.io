# LinkedIn Profile - English (Copy-Paste Ready)

---

## HEADLINE
Senior AI Engineer at AstraZeneca | Quantitative Software Architect | Research Software Engineering (RSE)

---

## LOCATION
Buenos Aires, Argentina

---

## ABOUT SECTION

I build production-grade AI and scientific software systems across research and high-complexity domains, combining quantitative rigor with strong engineering practice.

I currently work as **Senior AI Engineer at AstraZeneca (Oncology R&D)**. Before that, I spent 7+ years building mission-critical systems at JP Morgan, Qontigo/Axioma Risk, SimCorp, and Mercado Libre. My core strengths include:

**Quantitative Finance & Risk Systems**
• Multi-asset class pricing and valuation frameworks
• Risk analytics and portfolio management systems
• Model calibration, numerical methods, and RSE-driven validation (ensuring computational rigor)
• Performance optimization for computational finance

**Scientific Software Architecture**
• High-performance numerical computing (Python, C#, Julia)
• Domain-Specific Languages (DSLs) for financial modeling
• Research Software Engineering (RSE) Enablement: Standardizing analytical workflows from Jupyter to production using reproducibility standards and DevOps
• Migration and modernization of legacy quant systems
• Developer experience and tooling for analytical teams

**Specialized Training & Knowledge Transfer**
• Boutique Workshops: Research Software Engineering (RSE) Best Practices and High-Performance Numerical Computing with Julia
• Scientific and numeric software development training (best practices for data analysis, performance optimization, robust code design)
• Corporate workshops on quantitative finance and computational methods
• Technical mentorship for quant and engineering teams
• Interactive educational materials using Jupyter notebooks

My Physics background (Licenciatura, University of Buenos Aires) taught me to model complex problems by finding the right level of abstraction. Combined with production experience across tier-1 financial institutions, I deliver solutions that are both theoretically sound and operationally robust.

I specialize in the intersection where deep mathematical finance meets software engineering excellence—helping teams build systems that are correct, performant, and maintainable.

**How I Work:**
I offer flexible engagement models:
• Strategic Consulting: Architecture reviews, system design, technology evaluation
• Hands-On Development: POCs, migration projects, performance optimization
• Training & Enablement: Workshops, documentation, team upskilling

Whether you need to modernize a legacy valuation system, optimize computational performance, or build new analytical capabilities, I bring both strategic thinking and hands-on implementation skills.

**Open to consulting engagements globally (remote) and in Buenos Aires.**

---

## EXPERIENCE SECTION

### Independent Consultant
**Position Title:** Quantitative Software & Scientific Computing
**Employment Type:** Self-employed
**Company Name:** Independent Consultant
**Location:** Buenos Aires, Argentina
**Start Date:** January 2026
**End Date:** Present (check "I am currently working in this role")

**Description:**
Providing specialized consulting services in:
• Quantitative finance system architecture and development
• Scientific computing and numerical software optimization
• Technical leadership and team mentorship
• Developer experience and tooling for analytical platforms

Focus areas: Multi-asset pricing libraries, risk analytics systems, DSL development, performance optimization, Python/C#/Julia consulting.

---

### AstraZeneca
**Position Title:** Senior AI Engineer
**Employment Type:** Contract
**Company Name:** AstraZeneca
**Location:** Remote
**Start Date:** March 2026
**End Date:** Present (check "I am currently working in this role")

**Description:**
Advancing the R&D Agentic Platform and strengthening internal developer enablement for agent-based workflows.

**Key Responsibilities:**
• Enterprise Agentic Integrations: Architected and shipped the external integration surface of the R&D agentic platform, designing production-grade Model Context Protocol (MCP) servers with robust multi-tenant authentication patterns (OAuth 2.0 PKCE, Grant Chains, and Client Credentials)
• Competitive Intelligence & Knowledge Engines: Constructed automated document ingestion pipelines to continuously process and index enterprise sources into the LLM-Wiki/Knowledge Base; engineered hybrid search algorithms and optimized information retrieval (IR) traces to reduce latency and maximize source precision
• Automated Agent Evaluations (Evals): Spearheaded the design and deployment of automated evaluation frameworks measuring agent task-execution accuracy, tool-calling reliability, and retrieval stability across model updates
• Multi-Layer Memory & Personalization: Engineered user content personalization engines and structured agent memory systems integrating episodic, semantic, and working memory layers
• LLM Security & Threat Modeling: Audited tool-use capabilities in advanced reasoning architectures, identifying and neutralizing critical tool-injection vectors and token-abuse vulnerabilities before production
• Agentic Cost Engineering: Optimized execution graphs, achieving a 13%+ reduction in LLM token consumption through semantic pre-filtering and custom cache optimization layers

---

### Mercado Libre
**Position Title:** Technical Lead, Financial Planning & Analytics Engineering
**Employment Type:** Full-time
**Company Name:** Mercado Libre
**Location:** Buenos Aires Province, Argentina
**Start Date:** June 2025
**End Date:** March 2026

**Description:**
Leading technology strategy for platforms that optimize and standardize reporting, data workflows, and analytical processes across Latin America's largest e-commerce ecosystem. Managing two cross-functional engineering teams (Fintech & Cross, Commerce) with 14 engineers across backend, frontend, and data.

**Key Responsibilities:**
• Designing scalable architectures for financial planning and analytics systems
• Establishing engineering standards and best practices across FPA organization
• Leading development of unified variance-analysis platform for financial data
• Standardizing forecasting pipelines from ad-hoc Jupyter analyses to production workflows
• Mentoring engineers and managing hiring processes for team growth

**Technical Leadership:**
• Championing AI-assisted development workflows with agentic IDEs and LLMs
• Implementing code quality guidelines and automated validation pipelines
• Promoting modular design, clean architecture, and improved documentation standards
• Enhancing observability and debugging capabilities across data-driven systems

**Key Achievements:**
• Developed unified variance-analysis platform with streamlined data ingestion and transformation
• Transformed forecasting pipeline into reproducible, production-ready workflows
• Reduced cognitive load and manual overhead through improved abstractions
• Increased development velocity while maintaining strict quality standards

**Technologies:** Go, TypeScript, Python, BigQuery, Jupyter Notebook, cloud-native tooling, CI/CD, distributed systems

---

### SimCorp
**Position Title:** Lead Software Engineer, Core Analytics
**Employment Type:** Full-time
**Company Name:** SimCorp
**Location:** Buenos Aires Province, Argentina
**Start Date:** March 2024
**End Date:** May 2025

**Description:**
• Integrated Quant UI with Axioma Risk UI, enhancing analytical capabilities for institutional investors
• Developed POC for Domain-Specific Language (DSL) for pricing model interaction
• Created POC for interactive Q&A chatbot for product documentation using LLMs and RAG
• Redesigned libraries to support Automatic Differentiation (AD) and integrated third-party AD tooling

**Key Achievement:** Delivered interactive analytical tools that significantly improved quant developer productivity.

---

### Qontigo (formerly Axioma)
**Position Title:** Associate Principal, Core Analytics
**Employment Type:** Full-time
**Company Name:** Qontigo
**Location:** Buenos Aires, Argentina
**Start Date:** September 2020
**End Date:** March 2024

**Description:**

**Associate Principal (February 2023 - March 2024):**
• Managed junior developers and provided technical mentorship
• Maintained and developed core Quant Analytical libraries for multi-asset risk systems
• Designed and developed comprehensive curve construction library (rates, yields, discounts, spreads)
• Led development of interactive UI based on ipywidgets and voila for Quant library access
• Managed deployment of containerized applications to Azure

**Associate (September 2020 - February 2023):**
• Managed research internship on NeuralSDE applications for European Option Pricing using Julia, successfully applying RSE principles to ensure model robustness and computational reproducibility
• Designed and developed infrastructure for C# Monorepo for Analytical Libraries
• Extracted quantitative and numerical code from monolithic repository into reusable components

**Key Achievements:**
• Built APL (Axioma Pricing Library) from ground up—migrated complex day count conventions, bond mathematics, and curve construction with 100% accuracy
• Achieved 3x performance improvement in convertible bond pricing through cache optimization
• Received "Exceptional Performance" rating (2023) and "Strong Performance" ratings (2020-2022)

---

### J.P. Morgan
**Position Title:** Technology Analyst, Rates CIB
**Employment Type:** Full-time
**Company Name:** J.P. Morgan
**Location:** Buenos Aires
**Start Date:** July 2018
**End Date:** August 2020

**Description:**
• Developed infrastructure for migration of reporting services from Kapital (Smalltalk) to Athena (Python)
• Provided critical support to Rates Quant team, enhancing analytical capabilities
• Built production-grade financial reporting systems for interest rate derivatives

**Key Achievement:** Successfully migrated mission-critical reporting infrastructure while maintaining zero downtime.

---

### Open Source Contributor
**Position Title:** Scientific Software & Educational Tools
**Employment Type:** Volunteer
**Company Name:** Open Source Community
**Location:** Remote
**Start Date:** February 2016
**End Date:** Present (check "I am currently working in this role")

**Description:**
Active contributor to open source projects focused on:
• Interactive educational materials based on Jupyter notebooks
• Data analysis and visualization tools
• Scientific computing libraries

GitHub: @akielbowicz

---

## EDUCATION SECTION

### University of Buenos Aires
**School:** University of Buenos Aires
**Degree:** Licenciatura in Physics
**Field of Study:** Physics
**Start Date:** 2011
**End Date:** 2017
**Grade:** (leave blank or add if you have it)
**Activities and societies:** (leave blank)

**Description:**
Thesis: Statistical Analysis and Numerical Modeling of Single Particle Trajectories: Diffusion and Confinement Mechanisms

---

### 10 Pines
**School:** 10 Pines
**Degree:** Certificate
**Field of Study:** Software Engineering
**Start Date:** 2018
**End Date:** 2019

---

### Universidad del CEMA
**School:** Universidad del CEMA
**Degree:** Professional Certificate
**Field of Study:** Advanced Risk and Portfolio Management (ARPM)
**Start Date:** April 2021
**End Date:** November 2021

---

## SKILLS SECTION

Add these skills (LinkedIn allows up to 50 skills):

**Programming Languages:**
Python • C# • Julia • F# • OCaml • Clojure

**Quantitative Finance:**
Quantitative Finance • Risk Management • Portfolio Management • Derivatives Pricing • Multi-Asset Pricing • Model Calibration • Curve Construction • Financial Modeling • Quantitative Analytics

**Scientific Computing:**
Scientific Computing • Numerical Methods • Automatic Differentiation • Stochastic Differential Equations • High-Performance Computing • Mathematical Modeling • Optimization

**Software Architecture:**
Software Architecture • Microservices • API Design • Domain-Specific Languages (DSL) • Monorepo Infrastructure • Software Design

**Cloud & DevOps:**
Microsoft Azure • Docker • Containerization • CI/CD • GitHub Actions

**Tools & Technologies:**
Jupyter Notebook • Git • Visual Studio • ipywidgets

**Software Engineering:**
Software Development • Code Review • Technical Mentorship • Team Leadership • Performance Optimization

---

## PUBLICATIONS SECTION

### Publication 1
**Title:** Shared Memory Semi-Implicit Solver for Hydrodynamical Instability Processes
**Publisher:** Scientific Research Publishing
**Publication Date:** 2023
**URL:** https://www.scirp.org/journal/paperinformation?paperid=123858

---

### Publication 2
**Title:** Photon Counting Module based on Avalanche Photo-Diodes
**Publisher:** Anales AFA
**Publication Date:** 2017
**URL:** https://anales.fisica.org.ar/index.php/analesafa/article/view/2158

---

## LANGUAGES SECTION

**Spanish:** Native or bilingual proficiency
**English:** Professional working proficiency

---

## CONTACT INFO

**Website:** https://saxa.xyz/en
**Email:** augusto.kiel@gmail.com
**GitHub:** https://github.com/akielbowicz

---

## FEATURED SECTION (Optional - add links)

• Talks: http://talks.saxa.xyz/
• GitHub Profile: https://github.com/akielbowicz

---

## NOTES FOR LINKEDIN PROFILE SETUP

1. **Profile Photo:** Use a professional headshot
2. **Background Photo:** Consider using something related to scientific computing, finance, or technology
3. **Open to Work:** Enable "Open to work" feature with:
   - Job titles: Quantitative Consultant, Software Architect, Lead Engineer, Principal Engineer
   - Job types: Contract, Freelance, Full-time
   - Location: Remote, Buenos Aires
4. **Services:** Under "Providing services," add:
   - Quantitative Software Consulting
   - Scientific Computing Architecture
   - Technical Training & Workshops
5. **Creator Mode:** Consider enabling Creator Mode if you plan to share technical content
